...
function TerrainMesh({ heightMap }: TerrainMeshProps) {
  ...
  const geometry = useMemo(() => {
    ...
    for (let i = 0; i < positions.count; i++) {
      ...
      // Muted color gradient based on height (valley = dark blue-gray, peak = muted teal)
      const t = height;
      colors[i * 3] = 0.12 + t * 0.25;     // R
      colors[i * 3 + 1] = 0.18 + t * 0.45; // G
      colors[i * 3 + 2] = 0.28 + t * 0.35; // B
    }
...
function RaceMarker({ position, color, isUserPick, isLeader, designId, onHover }: RaceMarkerProps) {
  ...
  const markerSize = isUserPick ? 0.22 : isLeader ? 0.2 : 0.16;

  return (
    <group position={[worldPos.x, worldPos.y + 0.25, worldPos.z]}>
      {/* Main marker */}
      <mesh
      ...
      {isUserPick && (
        <mesh rotation={[Math.PI / 2, 0, 0]}>
          <torusGeometry args={[markerSize * 1.5, 0.02, 12, 24]} />
          <meshBasicMaterial color="#ffffff" transparent opacity={0.7} />
        </mesh>
      )}

      {/* Small indicator for leader */}
      {isLeader && (
        <mesh position={[0, markerSize + 0.1, 0]}>
          <coneGeometry args={[0.06, 0.1, 4]} />
          <meshStandardMaterial color="#d4a84b" metalness={0.3} roughness={0.7} />
        </mesh>
      )}
    </group>
  );
...
function Trail({ positions, color }: TrailProps) {
  const points = useMemo(() => {
    return positions.map(pos => {
      ...
      return new THREE.Vector3(world.x, world.y + 0.08, world.z);
    });
  }, [positions]);
...
function OptimalMarker() {
  ...
  return (
    <group position={[worldPos.x, worldPos.y + 0.4, worldPos.z]}>
      {/* Rotating diamond */}
      <mesh ref={meshRef}>
...
function LossLandscapeScene({ designs, userPickId, onHoverDesign, trails }: LossLandscapeSceneProps) {
  ...
  return (
    <>
      ...
      {/* Grid floor */}
      <gridHelper args={[12, 24, '#1a2028', '#1a2028']} position={[0, -0.01, 0]} />
